...
import { Send, Users, FileText, Eye, FlaskConical } from 'lucide-react';
...
export default function NotificationCreate({
    ...
    const recipientCount = useMemo(() => {
        const contactsFromGroups = new Set<number>();
        data.group_ids.forEach((groupId) => {
            const group = groups.find((g) => g.id === groupId);
            if (group) {
                contacts
                    .filter((c) => c.groups?.some((g) => g.id === groupId))
                    .forEach((c) => contactsFromGroups.add(c.id));
            }
        });
        data.contact_ids.forEach((id) => contactsFromGroups.add(id));
        return contactsFromGroups.size;
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <Tabs defaultValue="groups">
                                    ...
                                    <TabsContent
                                        value="contacts"
                                        className="space-y-3 mt-4 max-h-64 overflow-y-auto"
                                    >
                                        {contacts.length === 0 ? (
                                            <p className="text-sm text-muted-foreground">
